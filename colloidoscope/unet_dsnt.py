...
class UNetDSNT(nn.Module):
    ...
    def __init__(
        ...
        self.hm_conv = nn.Conv3d(out_channels, n_locations, kernel_size=1, bias=False)
...
class UNetCC(nn.Module):
    ...
    def __init__(
        ...
        if dim == 2:
            self.coordconv = CoordConv2d(in_channels, start_filters, 1, with_r=False)
        elif dim == 3:
            self.coordconv = CoordConv3d(in_channels, start_filters, 1, with_r=False)
